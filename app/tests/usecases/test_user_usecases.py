...
def test_register_user_username_already_exists(db_session):
    user_on_db = UserModel(username="Diogo", password=crypt_context.hash("pass#"))

    db_session.add(user_on_db)
    db_session.commit()

    uc = UserUseCases(db_session)
    user = User(username="Diogo", password=crypt_context.hash("pass#"))
...
def test_verify_token(db_session, user_on_db):
    uc = UserUseCases(db_session=db_session)
    data = {"sub": user_on_db.username, "exp": datetime.utcnow() + timedelta(minutes=30)}
